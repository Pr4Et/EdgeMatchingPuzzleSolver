...
if (use_files):
    ...
    node_features_df = pd.read_csv('solving/node_features.csv', header=None)
    node_features = torch.tensor(node_features_df.values, dtype=torch.float32)  # shape [256, node_dim]
    # Load edge type matrix from CSV
    edge_type_matrix_df = pd.read_csv('solving/edge_type_matrix.csv', header=None)
    edge_type_matrix = torch.tensor(edge_type_matrix_df.values, dtype=torch.long)  # shape [256, 256]
    initial_edge_type_matrix = torch.tensor(edge_type_matrix_df, dtype=torch.float).to(device) #not realy, just to avoid error
else:
    ...
    updated_pairs_mat = update_pairs_mat(pairs_mat, node_features, 0.01)
...
with torch.no_grad():
    ...
    predicted_clean_edge_type_matrix = (edge_logits > threshold).int()
     #predicted_clean_edge_type_matrix = torch.argmax(logits, dim=-1)
...
print(f"Loaded edge count: {(edge_type_matrix > 0).sum().item()}, Predicted edge count: {(predicted_clean_edge_type_matrix > 0).sum().item()}, correct predictions:  {((predicted_clean_edge_type_matrix>0) & (initial_edge_type_matrix>0)).sum().item()} ")

#print(predicted_clean_edge_type_matrix)   #piece numbers between 0 and 255
